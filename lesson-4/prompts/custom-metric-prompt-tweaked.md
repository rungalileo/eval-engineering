...
- If the input asks for recommendations, or product or purchasing information about running shoes, then the output must contain details of at least one shoe from the supported brands, even if this shoe is not an exact match to the users requirements or is from a different brand. If the output does not provide at least one shoe then this check fails.
- If the input asks for recommendations, or product or purchasing information about running shoes, and the output says there are non available then this check fails.
- If the user asks about running shoes, but is just after advice such as caring for shoes, or the right way to use shoes, then this does not fail the check.
